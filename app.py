...
async def send_multiple_requests(uid, server_name, url):
    try:
        ...
        # ✅ RANDOM TOKENS
        selected_tokens = random.sample(tokens, min(225, len(tokens)))
...
def fetch_player_info(uid):
    try:
        url = f"https://infosemy.vercel.app/semy-info?uid={uid}&region=IND"
        response = requests.get(url, timeout=5)

        if response.status_code == 200:
            data = response.json()

            basic = data.get("basicInfo", {})   # ⭐ correct variable

            return {
                "Level": basic.get("level", "NA"),
                "Region": basic.get("region", "NA"),
                "ReleaseVersion": basic.get("releaseVersion", "NA")
            }
        else:
            app.logger.error(f"Player info API failed: {response.status_code}")
